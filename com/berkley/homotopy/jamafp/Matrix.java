...
Edited by Victor Shia for Fixed Point l1-Homotopy Implementation
 */
...
public class Matrix implements Cloneable, java.io.Serializable {
   ...
   @return     replace A by s*A
	...
	public Matrix timesEquals (int s) {
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
...
*/
   ...
   Private Methods
	 ...
	 * My methods
	 */
	public Matrix tranposeAndNegate(){
		Matrix X = new Matrix(n,m);
		int[][] A = getArray();
		int[][] C = X.getArray();
		for (int i = 0; i < m; i++) {
	...
	public static Matrix append(Matrix topleft, Matrix topright, Matrix botleft, Matrix botright){
		int m = topleft.getRowDimension() + botright.getRowDimension();
		int n = topleft.getColumnDimension() + botright.getColumnDimension();
		Matrix rtn = new Matrix(m,n);
		rtn.setMatrix(0, topleft.getRowDimension()-1  , 0, topleft.getColumnDimension()-1, topleft);
		rtn.setMatrix(topleft.getRowDimension(), m-1  , 0, botleft.getColumnDimension()-1, botleft);
		rtn.setMatrix(0, topright.getRowDimension()-1 , topleft.getColumnDimension(), n-1, topright);
		rtn.setMatrix(topleft.getRowDimension(), m-1  , topleft.getColumnDimension(), n-1, botright);
		return rtn;
	}


	public Matrix divide (int s) {
		Matrix X = new Matrix(m,n);
		int[][] C = X.getArray();
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < n; j++) {
